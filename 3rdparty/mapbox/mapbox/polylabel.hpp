...
template <class T>
T polylabel_distance(const geometry::polygon<T>& polygon, T &xx, T &yy, T precision = 1, bool debug = false) {
	using namespace detail;
...
template <class T>
T polylabel_distance(const geometry::polygon<T>& polygon, T precision = 1, bool debug = false) {
	T x, y;
	return polylabel_distance(polygon, x, y, precision, debug);
}
