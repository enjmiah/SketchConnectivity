...
void Parameterization::parameterize_cluster(Cluster* cluster) {
	...
	const auto record_current_u = [&]() -> void {
		prev_u.clear();
	...
	const auto converged = [&]() -> bool {
		// Immediately converge if we only have a single stroke as the input (equivalent to sampling)
	...
	for (int it = 0; it < 5; ++it) {
		...
		if (converged()) {
			...
			for (auto& xsec : cluster->xsecs) {
				...
				const auto get_u = [&](size_t idx) -> double {
					auto& pt = xsec.points[idx];
					double mix = pt.i - std::floor(pt.i);
					return (1. - mix) * cluster->strokes[pt.stroke_idx].u[(size_t)std::floor(pt.i)] +
						mix * cluster->strokes[pt.stroke_idx].u[(size_t)std::ceil(pt.i)];
				};
				for (auto& connection : xsec.connections) {
	...
	for (int it = 0; it < 5 && cluster->strokes.size() > 1; ++it) {
		record_current_u();
	...
	if (cluster->periodic && cluster->strokes.size() > 1) {
		auto old_u = [&](size_t stroke, double i) {
			double mix = i - std::floor(i);
			return (1. - mix) * prev_u[stroke][(size_t)std::floor(i)] + mix * prev_u[stroke][(size_t)std::ceil(i)];
		};
		auto is_cut = [&](const Cluster::XSec& xsec) {
			for (auto& conn : xsec.connections) {
				if (std::abs(
					...
					old_u(xsec.points[conn.b_idx].stroke_idx, xsec.points[conn.b_idx].i)) > 5.) {
					return true;
				}
			}
			return false;
...
void Parameterization::params_from_xsecs(Cluster* cluster, bool initial, Cluster::XSec* cut) {
	// If there's only a single stroke
	if (cluster->strokes.size() == 1 || !context.grb_ptr) {
		for (size_t i = 0; i < cluster->strokes[0].points.size(); ++i) {
			if (i == 0)
				cluster->strokes[0].u[i] = 0;
			else
				cluster->strokes[0].u[i] = cluster->strokes[0].u[i - 1] +
				glm::distance(cluster->strokes[0].points[i], cluster->strokes[0].points[i - 1]);
		}
		return;
	}
	...
	for (auto& xsec : cluster->xsecs) {
		...
		// 1. Add velocity term
		{
			...
			for (size_t i = 0; i < xsec.points.size(); ++i) {
				...
				auto a = (int)std::floor(point.i);
				auto b = (int)std::ceil(point.i);
		...
		if (!cut || !std::any_of(xsec.connections.begin(), xsec.connections.end(), connection_crosses_cut)) {
			for (auto& connection : xsec.connections) {
				...
				GRBLinExpr u_a = (1 - mix_a) * param_vars[pt_a.stroke_idx][(size_t)std::floor(pt_a.i)] +
					mix_a * param_vars[pt_a.stroke_idx][(size_t)std::ceil(pt_a.i)];

				double mix_b = pt_b.i - std::floor(pt_b.i);
				GRBLinExpr u_b = (1 - mix_b) * param_vars[pt_b.stroke_idx][(size_t)std::floor(pt_b.i)] +
					mix_b * param_vars[pt_b.stroke_idx][(size_t)std::ceil(pt_b.i)];
	...
	for (size_t stroke_idx = 0; stroke_idx < cluster->strokes.size(); ++stroke_idx) {
		...
		for (size_t i = 0; i < stroke.points.size() - 1; ++i) {
			if (cut && points_cross_cut(stroke_idx, (int)i, int(i + 1))) continue;
	...
	if (cut) {
		for (auto& pt : cut->points) {
			...
			model.addConstr(param_vars[pt.stroke_idx][(size_t)std::ceil(pt.i)] == dist_from_cut);
		}
	}
	else {
	...
	for (size_t stroke_idx = 0; stroke_idx < cluster->strokes.size(); ++stroke_idx) {
		...
		for (size_t i = 0; i < stroke.points.size(); ++i) {
			try {
				...
				std::cout << "params_from_xsecs: Error code = " << e.getErrorCode() << "; " << std::to_string(i) << " / " << std::to_string(stroke.points.size()) << std::endl;
				std::cout << e.getMessage() << std::endl;
				// throw e;
...
std::vector<Cluster::XSec> Parameterization::orthogonal_xsecs(const Cluster& cluster, double angle_tolerance) {
	...
	// 1. Angle filtering
	{
		...
		for (auto& xsec : xsecs) {
			...
			for (int i = (int)xsec.center_idx + 1; i < xsec.points.size(); ++i) {
			...
			for (int i = (int)xsec.center_idx - 1; i >= 0; --i) {
				if (std::acos(glm::dot(xsec.points[i + 1].tangent, xsec.points[i].tangent)) > max_angle) {
	...
	// 2. Gap filtering
	{
		...
		for (auto& xsec : xsecs) {
			...
			auto median_offset = size_t(gaps.size() * 0.75);
			std::nth_element(
				gaps.begin(),
...
Cluster::XSec Parameterization::orthogonal_xsec_at(const Cluster& cluster, size_t stroke, double i) {
	...
	for (size_t j = 0; j < cluster.strokes.size(); ++j) {
		...
		for (auto& intersection : ints) {
			...
			auto floor_other_i = (size_t)std::floor(intersection.i);
	...
	if (potential_ints.size() > 1) {
		...
		for (int n = (int)potential_ints.size() - 1; n >= 0; --n) {
			if (potential_ints[n].signed_dist < 0 && glm::dot(potential_ints[n].point.tangent, tan) < 0) {
				first_bad_tangent = n;
				break;
			}
		}
		auto last_bad_tangent = (int)potential_ints.size();
	...
	for (size_t n = 0; n < num_xsecs; ++n) {
		...
		if (xsec.points.size() > 0) {
			...
			for (auto& pt : xsec.points) {
				sampled[pt.stroke_idx][(size_t)std::floor(pt.i)] = true;
				sampled[pt.stroke_idx][(size_t)std::ceil(pt.i)] = true;
			}
		}
	}
...
void Parameterization::ensure_connected(Cluster* cluster, Cluster::XSec* cut) {
	...
	for (size_t stroke_idx = 0; stroke_idx < cluster->strokes.size(); ++stroke_idx) {
		...
		for (double cut_i : cuts) {
			segment.end = cut_i;
			segment.small = std::min<int>(cluster->strokes[stroke_idx].points.size() - 1, (int)std::floor(segment.end)) -
				std::max<int>(0, (int)std::ceil(segment.begin)) <= 2;
	...
	while (!ok) {
		...
		for (auto& disconnected : connected) {
			...
			for (auto& kv : connected) {
				...
				for (int ia : { std::max<int>(0, (int)std::ceil(min_i)), std::min<int>(cluster->strokes[stroke_a].points.size() - 1, (int)std::floor(max_i)) }) {
					for (int ib : { std::max<int>(0, (int)std::ceil(min_j)), std::min<int>(cluster->strokes[stroke_b].points.size() - 1, (int)std::floor(max_j)) }) {
...
void Parameterization::check_periodic(Cluster* cluster) {
	...
	for (int xsec_idx = 0; xsec_idx < cluster->xsecs.size(); xsec_idx += 3) {
		...
		for (auto& conn : xsec.connections) {
			...
			auto ia = (int)std::round(raw_ia);
			auto ib = (int)std::round(raw_ib);
	...
	const auto find_path = [&](const Node& from, const Cluster::XSec& to) -> std::vector<int> {
		int allowed_jumps = 2;
	...
	auto get_u = [&](const Cluster::XSecPoint& pt) {
		double mix = pt.i - std::floor(pt.i);
		return (1. - mix) * cluster->strokes[pt.stroke_idx].u[(size_t)std::floor(pt.i)] + mix * cluster->strokes[pt.stroke_idx].u[(size_t)std::ceil(pt.i)];
	};
	...
	auto off = int(0.2 * (period_samples.size() - 1));
