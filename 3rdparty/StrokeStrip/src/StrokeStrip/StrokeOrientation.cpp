...
std::vector<int> StrokeOrientation::orient_cluster_strokes(const Cluster& cluster) {
	...
	for (size_t i = 0; i < cluster.strokes.size(); i++) {
		try {
			...
			std::cout << "orient_cluster_strokes: Error code = " << e.getErrorCode() << "; " << std::to_string(i) << " / " << std::to_string(cluster.strokes.size()) << std::endl;
			std::cout << e.getMessage() << std::endl;
			// throw e;
			cluster_orientations.clear();
			break;
		}
	}
...
StrokeOrientation::PolicyResult StrokeOrientation::evaluate_policy(
	...
	// New: Filter out pairs that are too long
	double max_length = std::numeric_limits<double>::infinity();
	double jump_ratio = 5;
	std::vector<double> overlapping_lengths;
	for (size_t i = 0; i < overlaps_a.size(); ++i) {
		if (overlaps_a[i] == -1) continue;
		double d = glm::distance(a.points[i], b.points[overlaps_a[i]]);
		overlapping_lengths.emplace_back(std::max(d, std::max(a.max_width, b.max_width)));
	}
	for (size_t j = 0; j < overlaps_b.size(); ++j) {
		if (overlaps_b[j] == -1) continue;
		double d = glm::distance(a.points[overlaps_b[j]], b.points[j]);
		overlapping_lengths.emplace_back(std::max(d, std::max(a.max_width, b.max_width)));
	}
	std::sort(overlapping_lengths.begin(), overlapping_lengths.end());
	for (size_t i = 0; i + 1 < overlapping_lengths.size(); ++i) {
		if (overlapping_lengths[i] * jump_ratio < overlapping_lengths[i + 1])
			max_length = overlapping_lengths[i + 1];
	}
	for (size_t i = 0; i < overlaps_a.size(); ++i) {
		if (overlaps_a[i] == -1) continue;
		double d = glm::distance(a.points[i], b.points[overlaps_a[i]]);
