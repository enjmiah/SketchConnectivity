  ...
  for (const auto& j : final_predictions) {
    ...
    if ((sorted_junc.type == JunctionType::R &&
      ...
      break;
               ...
               sorted_junc.points[1] == sorted_j.points[1]) {
      Float diff = std::abs(sorted_junc.points[0].second - sorted_j.points[0].second);
      if (diff < time_diff) {
        time_diff = diff;
        closest_junc = sorted_j;
      }
...
bool should_delay(const StrokeGraph& final_stroke_graph, const Junction& junc,
  ...
  // Check the end containing original stroke if any snapped vertices is close to this end
  auto is_vertex_close = [&final_stroke_graph](const StrokeTime& end, size_t exclude_si) {
    ...
    if (s_end.second != 0.0 && s_end.second != 1.0)
      return false;
    const auto v =
      endpoint_to_vertex(final_stroke_graph, Endpoint(s_end.first, s_end.second == 0.0));
    assert(v.is_valid() && v.is_active());
    if (v.is_dangling())
      return false;

    // Check if the stroke is snapped to a stroke that is not the one in this proposed
    // candidate
...
bool is_visible_in_final(span<const Stroke> strokes, const Vec2 p, const Vec2 q) {
  Float ss, t;
...
bool is_final_prediction_acceptable(const Junction& junc,
                                    const std::vector<Junction>& final_predictions) {
...
void get_stroke_idx_round(const StrokeGraph& graph, const std::pair<int, double> end,
    ...
    if (d1 < dist) {
      ...
      best_ends.emplace_back(
        Endpoint{si_mapping.first, si_mapping.second.range_arclens_[0] < 0.5});
               ...
               d1 < 1e-10) { // So we only match both continuous edges if at the vertex
      best_ends.emplace_back(
        Endpoint{si_mapping.first, si_mapping.second.range_arclens_[0] < 0.5});
    }
    ...
    if (d2 < dist) {
      ...
      best_ends.emplace_back(
        Endpoint{si_mapping.first, si_mapping.second.range_arclens_[1] < 0.5});
               ...
               d2 < 1e-10) { // So we only match both continuous edges if at the vertex
      best_ends.emplace_back(
        Endpoint{si_mapping.first, si_mapping.second.range_arclens_[1] < 0.5});
    }
  }
...
bool is_high_valence_junction_on_cycle(const StrokeGraph& graph,
                                       const span<const Junction> candidates, size_t fi,
  ...
  // It's possible if a junction is removed since one of its two corresponding original
  // stroke is removed
  if (cand_idx < 0)
    return false;
  ...
  // Use seen_pairs to handle the special case of a single edge connected to itself
  // forming a cycle. In this case, we would check the same edge pair twice.
  ...
  for (const auto& c : face_edge_ends) {
    ...
    for (size_t i = 0; i < c.size(); ++i) {
      ...
      auto p =
        std::make_pair(std::min(i, (i + 1) % c.size()), std::max(i, (i + 1) % c.size()));
...
void update_high_valence_prediction(StrokeGraph& varying_graph,
  ...
  std::unordered_set<int> junc_sid{hv_junc.points[0].first, hv_junc.points[1].first};
  ...
  update_disconnected_junction_predictions(varying_graph, junc, prediction_feature_type,
                                           false, true);
...
bool graph_visible(const StrokeGraph& stroke_graph, const Junction& in_junc) {
  ...
  Vec2 p = stroke_graph.strokes_[junc.points[0].first].pos_norm(junc.points[0].second);
  Vec2 q = stroke_graph.strokes_[junc.points[1].first].pos_norm(junc.points[1].second);
  return line_of_sight(p, q, stroke_graph.strokes_, stroke_graph.bvh_.centerline_bbs());
}

bool envelope_visible(const StrokeGraph& stroke_graph, const Junction& junc) {
  Vec2 p =
    stroke_graph.orig_strokes_[junc.points[0].first].pos_norm(junc.points[0].second);
  Vec2 q =
    stroke_graph.orig_strokes_[junc.points[1].first].pos_norm(junc.points[1].second);
...
bool corner_projection_check(
  ...
  for (const auto& proj_junc : bind_junc) {
    ...
    const auto dist = //
      (proj_junc.points[0].first == proj_junc.points[1].first)
        ? closest_point_to_own_stroke(stroke_graph.orig_strokes_[orig_si],
                                      proj_junc.points[1].first < 0.5, proj, arclen)
        : closest_point(
            stroke_graph.orig_strokes_[orig_si],
            stroke_graph.orig_strokes_[dangling_si].pos_norm(proj_junc.points[1].second),
            proj, arclen);
    ...
    if (!corner_v.is_valid()) {
      close_to_all &=
        end_stroke_junction_type(stroke_graph.orig_strokes_[orig_si], end_arclen, arclen,
                                 corner_projection_ratio) == 0;
      // If the rounded vertex is close to the endpoint
      close_to_all &=
        end_stroke_junction_type(stroke_graph.orig_strokes_[orig_si], end_arclen,
...
bool trial_add_corner_junction(const StrokeGraph& end_stroke_graph,
  ...
  if (has_bridge_vertex(end_stroke_graph, junc_round))
    return false;
  ...
  if ( // end_stroke_junction_type(s, s.length() * s_junc.points[0].second,
    ...
    end_stroke_junction_type(
      orig_s, (junc_round.points[0].second < 0.5) ? 0 : orig_s.length(),
      orig_s.length() * junc_round.points[0].second, corner_projection_ratio) == 0 &&
...
bool high_valence_dangling_visible(const StrokeGraph& stroke_graph,
  ...
  const auto v = endpoint_to_vertex(
    stroke_graph, Endpoint(s_junc.points[0].first, s_junc.points[0].second == 0.0));
...
void add_e2e_corner_junction(const StrokeGraph& end_stroke_graph,
                             ...
                             std::vector<Junction>& corner_predictions) {
  if (!has_bridge_vertex(end_stroke_graph, junc) &&
      high_valence_dangling_visible(end_stroke_graph, junc)) {
    Junction s_junc = junc;
    auto ok = original_stroke_to_stroke_indexing(stroke_graph, s_junc);
    force_assert(ok && "couldn't map from orig to strokes");
    ...
    junc_round.points[0].second = (junc_round.points[0].second < 0.5) ? 0.0 : 1.0;
    ...
    auto corner_close = corner_projection_check(stroke_graph, junc_round, false, v);
    // If we are generating corners for reconsideration with end-end classifier
    // predictions, keep the projection test.
    if ((&end_stroke_graph == &stroke_graph && recosider_type == JunctionType::R) ||
        corner_close) {
...
bool connection_corner_visible(const StrokeGraph& stroke_graph, const Junction& junc) {
  // Find the two adjacent strokes
  std::vector<Junction> bind_junc;
  expand_adjacent_junction(stroke_graph, junc, bind_junc);
    ...
    for (const auto& end : j.points) {
      if (end != junc.points[1]) {
        if (end.second == 0.0 || end.second == 1.0)
          ...
          seen_interior = true;
        Vec2 p =
          stroke_graph.orig_strokes_[j.points[0].first].pos_norm(j.points[0].second);
        Vec2 q =
          stroke_graph.orig_strokes_[j.points[1].first].pos_norm(j.points[1].second);
        if (!line_of_sight(p, q, *stroke_graph.orig_bvh_) ||
...
void print_junctions(const std::vector<Junction>& candidates) {
  ...
  for (const auto& junc : candidates)
    SPDLOG_INFO("Junc: {}, {} - {}, {}:\n\t{} comp. {}; {}; {}; {}", junc.points[0].first,
                junc.points[0].second, junc.points[1].first, junc.points[1].second,
                junc.component_idx, junc.probability,
                (junc.type == JunctionType::R) ? "R" : "T",
                (junc.repr.empty()) ? "Dis" : "Conn", (junc.is_weak) ? "WEAK" : "");
}
...
bool has_bridge_vertex(const StrokeGraph& stroke_graph, const Junction& junc) {
  for (const auto& p : junc.points) {
    ...
    if (std::abs(s_end.second) >= 1e-10 && std::abs(s_end.second - 1.0) >= 1e-10)
      continue;

    s_end.second = (s_end.second < 0.5) ? 0.0 : 1.0;
    const auto v =
      endpoint_to_vertex(stroke_graph, Endpoint(s_end.first, s_end.second == 0.0));
    auto is_bridge = is_bridge_vertex(stroke_graph, v);
    if (is_bridge)
      return true;
  }

  // If it's too close to one
  if (junc.type == JunctionType::T) {
    StrokeGraph tmp_graph = stroke_graph.clone();
    StrokeTime s_end = junc.points[0];
    auto ok = convert_orig2strokes(stroke_graph, s_end);
    tmp_graph.strokes_[s_end.first].ensure_arclengths();
    StrokeGraph::VertexView new_vertex;
    add_vertex(tmp_graph, s_end.first,
               s_end.second * tmp_graph.strokes_[s_end.first].length(), new_vertex);
    if (is_bridge_vertex(tmp_graph, new_vertex))
      return true;
  }
...
bool is_snap_valid(const StrokeGraph& stroke_graph, const Junction& junc) {
  ...
  // Special case of snapping two copies of the same vertices. This is considered valid
  // since this can emerge naturally in sequenctially snapping high-valence vertices.
  if (vert == vert2)
    return true;
...
bool is_high_valence_junction_on_boundary_cycle(const StrokeGraph& graph,
                                                ...
                                                const StrokeGraph::VertexView v,
                                                const std::string& vid_str) {
  return is_high_valence_junction_on_cycle(graph, candidates, fi, v, vid_str, true);
}
...
bool is_high_valence_junction_in_region_cycle(const StrokeGraph& graph,
                                              ...
                                              const std::string& vid_str) {
  return is_high_valence_junction_on_cycle(graph, candidates, fi, v, vid_str, false);
}
...
bool is_candidate_valid(const StrokeGraph& stroke_graph, span<const Stroke> strokes,
                        ...
                        const Junction& junc,
                        const std::vector<Junction>& final_predictions) {
  if (has_bridge_vertex(stroke_graph, junc))
    return false;

  // Use the stroke based indexing for the position since the original strokes may have
  // been deformed
  Junction s_junc = junc;
  ...
  // Use the current strokes in the graph and all the future strokes for the intersection
  // test
  ...
  if (junc.probability > std::numeric_limits<double>::epsilon() &&
      ...
      line_of_sight(p, q, stroke_graph.strokes_, stroke_graph.bvh_.centerline_bbs()) &&
      is_visible_in_final(span<const Stroke>(strokes.begin() + cur_stroke_step + 1,
...
void expand_junction(const StrokeGraph& graph, const Junction& in_junc,
  ...
  if (junc.type == JunctionType::R) {
    ...
    expand_candidates_vv(graph, c1, c2, {&expanded_cand1[0], n}, {&expanded_cand2[0], n});
    ...
    for (size_t j = 0; j < n; ++j) {
      ...
      Junction new_junc(
        std::vector<std::pair<int, double>>{
          std::make_pair((int)snap_info.stroke_pos2.first, snap_info.stroke_pos2.second),
          std::make_pair((int)snap_info.stroke_pos1.first, snap_info.stroke_pos1.second)},
...
void expand_adjacent_junction(const StrokeGraph& graph, const Junction& junc_,
  ...
  if (!is_snap_valid(varying_graph, v.index_, dangling_v.index_))
    return;
  bool snapped = varying_graph.snap_vertices(v.index_, dangling_v.index_,
                                             varying_graph.snapping_method_type_);
  if (!snapped)
    return;
  ...
  // Convert the dangling-continous pairs to a single T-junction (so the region code can
  // handle it)
  if (bind_junc.size() == 2 &&
      varying_graph.hedge(out_he_idx.front()).continuity() == out_he_idx.back()) {
...
bool update_disconnected_junction_predictions(const StrokeGraph& stroke_graph,
                                              ...
                                              FeatureType feature_type, bool to_expand,
                                              bool use_input_junc_type) {
  ...
  // Because not all candidates will be valid, we need to map from `varying_candidates`
  // indices into a possibly reduced size cand1 and cand2.
  ...
  for (size_t i = 0; i < varying_candidates.size(); ++i) {
    ...
    const auto could_map = original_stroke_to_stroke_indexing(stroke_graph, pred_junc);
    if (!could_map || !pred_junc.is_valid() || !is_snap_valid(stroke_graph, junc) ||
        has_continuous_edge(
          endpoint_to_vertex(stroke_graph, Endpoint(pred_junc.points[1].first,
                                                    pred_junc.points[1].second < 0.5)))) {
      if (!junc.is_weak)
        SPDLOG_WARN("Incorrect junction: {}, {} - {}, {} => {}, {} - {}, {}",
                    junc.points[0].first, junc.points[0].second, junc.points[1].first,
                    junc.points[1].second, pred_junc.points[0].first,
                    pred_junc.points[0].second, pred_junc.points[1].first,
                    pred_junc.points[1].second);
      all_valid = false;
      indices.push_back((size_t)-1);
      if (junc.is_weak)
        junc.must_disconnect = true;
    } else {
      cand1.emplace_back(pred_junc.points[1].first, pred_junc.points[1].second < 0.5);
...
bool update_high_valence_junction_predictions(const StrokeGraph& stroke_graph,
  ...
  update_disconnected_junction_predictions(stroke_graph, varying_candidates, feature_type,
                                           false);
  ...
  for (size_t i = 0; i < varying_candidates.size(); ++i) {
    ...
    const auto junc_p = std::make_pair(std::min(to_v_idx(stroke_graph, junc.points[0]),
                                                to_v_idx(stroke_graph, junc.points[1])),
                                       std::max(to_v_idx(stroke_graph, junc.points[0]),
                                                to_v_idx(stroke_graph, junc.points[1])));
  ...
  for (auto& [p, juncs] : high_valence_junctions) {
    if (juncs.size() == 1 && varying_candidates[juncs.front()].point_tos.empty()) {
    ...
    for (const auto i : juncs) {
      const auto& junc = varying_candidates[i];
      const auto& v =
        varying_stroke_graph.vertex(to_v_idx(varying_stroke_graph, junc.points[0]));
...
void complete_candidate_graph(const StrokeGraph& graph,
  ...
  auto find_endpoint_time = [&endpoint_times](const double junc_hash) -> size_t {
  ...
  for (size_t i = 0; i < candidates.size(); ++i) {
    ...
    for (const auto& end : junc.points) {
      ...
      if (s_end.second == 0.0 || s_end.second == 1.0) {
        auto v = endpoint_to_vertex(graph, Endpoint(s_end.first, s_end.second == 0.0));
        if (!v2ends.count(v.index_))
          v2ends[v.index_] = std::vector<double>();
        if (std::find(v2ends[v.index_].begin(), v2ends[v.index_].end(), junc_hash) ==
            v2ends[v.index_].end())
    ...
    for (size_t i = 0; c >= 0 && i + 1 < components.size(); ++i) {
      ...
      for (size_t j = i + 1; j < components.size(); ++j) {
        if (components[j] != c)
          continue;
        Junction junc(
          std::vector<std::pair<int, double>>{endpoint_indexing[i], endpoint_indexing[j]},
          JunctionType::R, true, 0.5, true);
        junc.sort_entries();
        auto s_end = junc.points[0];
        bool ok = convert_orig2strokes(graph, s_end);
        force_assert(ok && "couldn't map from orig to strokes");
        junc.type = (s_end.second < 1e-10 || std::abs(1.0 - s_end.second) < 1e-10)
                      ? JunctionType::R
                      : JunctionType::T;
        auto s_end2 = junc.points[1];
        ok = convert_orig2strokes(graph, s_end2);
        force_assert(ok && "couldn't map from orig to strokes");
        // if (!(s_end2.second < 1e-10 || std::abs(1.0 - s_end2.second) < 1e-10))
        //  junc.must_disconnect = true;

        // Avoid creating interior-interior junctions since it's discarded even before
        // graph modification
        if (junc.type == JunctionType::T &&
        ...
        if (junc.type == JunctionType::R) {
          const auto v1 =
            endpoint_to_vertex(graph, Endpoint(s_end.first, s_end.second < 0.5));
          const auto v2 =
            endpoint_to_vertex(graph, Endpoint(s_end2.first, s_end2.second < 0.5));
            ...
            return (junc_.type == junc.type) &&
                   (((junc_.points[0].first == junc.points[0].first) &&
                    ...
                    ((junc_.points[0].first == junc.points[1].first) &&
                     (std::abs(junc_.points[0].second - junc.points[1].second) < 1e-10) &&
                     (junc_.points[1].first == junc.points[0].first) &&
                     (std::abs(junc_.points[1].second - junc.points[0].second) < 1e-10)));
          });
        // Add to the result set
        if (junc_itr == candidates.end()) {
          auto v1 =
            endpoint_to_vertex(graph, Endpoint{(size_t)s_end.first, s_end.second < 0.5});
...
bool reproject_t_junc(const StrokeGraph& graph, Junction& junc) {
  ...
  // Reproject and redo the line-of-sight check to account for deformed geometry.
  ...
  if (!line_of_sight(strokes[cand.points[1].first].pos_norm(cand.points[1].second),
...
bool reproject_t_junc_orig(const StrokeGraph& graph, Junction& junc) {
  ...
  if (junc.points[1].second == 0.0 || junc.points[1].second == 1.0) {
    v = graph.orig_strokes_[junc.points[1].first].pos_norm(junc.points[1].second);
    is_head = junc.points[1].second < 0.5;
    si = 0;
    v_si = 1;
  } else {
    v = graph.orig_strokes_[junc.points[0].first].pos_norm(junc.points[0].second);
  ...
  Vec2 _proj;
  Float s;
  const auto dist = ((junc.points[0].first == junc.points[1].first)
  ...
  if (!line_of_sight(strokes[v_si].pos_norm(v_s), strokes[si].pos(s), *graph.orig_bvh_)) {
    return false;
  }
  ...
  for (size_t i = 0; i < strokes.size(); ++i) {
    ...
    for (size_t j = 0; j < candidates.size(); ++j) {
      ...
      if (max_sid == i) {
        ...
        // Don't return false directly if reproject_t_junc fails. Let modify_graph
        // determine that.
        if (junc.type == JunctionType::T)
          reproject_t_junc(stroke_graph, junc);
        std::vector<Junction> connected_junc{junc};
        auto modified_graph =
          modify_graph(stroke_graph, connected_junc, std::vector<bool>{true}, adj_faces,
...
bool replay_connection(const std::vector<Junction>& candidates,
  ...
  for (const auto& [c, indices] : group_indices) {
    ...
    for (const auto i : indices) {
      ...
      connected_junc.back().points[0] = reprojections[connected_junc.back().points[0]];
    }
    ...
    std::vector<Float> junc_distances;
    std::vector<StrokeGraph::VertexID> junc_vertex_ids;
    auto modified_graph = modify_graph(stroke_graph, connected_junc, connected_flags,
...
void color_endpoint_graph(const StrokeGraph& stroke_graph,
                          ...
                          std::vector<int>& junc_components, const Float min_prob) {
  // 1. Build an endpoint graph
  std::unordered_map<size_t, std::vector<double>> v2ends;
  std::vector<double> endpoint_times;
  endpoint_times.reserve(candidates.size() * 2);
  ...
  for (size_t i = 0; i < candidates.size(); ++i) {
    ...
    // If given a threshold, also ignore the weak junctions (the ones used to complete the
    // component graph)
...
void trim_overshoots(StrokeGraph& stroke_graph) {
  ...
  for (size_t vi = 0; vi < stroke_graph.vertices_.size(); ++vi) {
    ...
    if (v.is_active() && v.is_dangling()) {
      ...
      auto end = Endpoint(v.hedge().stroke_idx(), v.hedge().forward()).as_pair();
      ...
      if (d2 < 1.5 * 2 * vertex_radius(dest) &&
        ...
        // stroke_graph.vertices_[vi].flags_ |= StrokeGraph::VertexRecord::Overshoot;
      }
    }
  }
...
void mark_missed_overlapping_endpoints(StrokeGraph& stroke_graph) {
  ...
  for (size_t vi = 0; vi < stroke_graph.vertices_.size(); ++vi) {
    ...
    if (vert.is_active() && vert.is_dangling()) {
      auto orig_pos =
        StrokeTime((int)vert.hedge().stroke_idx(), vert.hedge().forward() ? 0.0 : 1.0);
      const auto ok = convert_strokes2orig(stroke_graph, orig_pos);
      force_assert(ok && "couldn't map from strokes to orig");
      force_assert(orig_pos.second == 0.0 || orig_pos.second == 1.0);

      for (size_t orig_si = 0; orig_si < stroke_graph.orig_strokes_.size(); ++orig_si) {
        ...
        const auto& this_orig_stroke = stroke_graph.orig_strokes_[orig_pos.first];
        const auto [cen_dist, arclen] = find_snap_point(
          stroke_graph.orig_strokes_[orig_pos.first], /*head=*/orig_pos.second == 0.0,
        ...
        if (cen_dist < INFINITY) {
          ...
          if (!shares_intersection ||
            ...
            stroke_graph.vertices_[vi].flags_ |= StrokeGraph::VertexRecord::Overlapping;
            break;
          }
        } else {
          // Check for self-loop.
          ...
          const auto loop_env_dist = loop_cen_dist - 0.5 * (this_orig_stroke.width(0) +
                                                            this_orig_stroke.width(Back));
          if (loop_env_dist <= 0 &&
  ...
  for (size_t vi = 0; vi < stroke_graph.vertices_.size(); ++vi) {
    ...
    if (v.valence() == 2) {
      ...
      for (const auto& snap : stroke_graph.snap_history_) {
        assert(snap.key.cand1 != (int)v.index_); // v.index_ is always going to be cand2.
        if (snap.key.type == JunctionType::R && snap.key.cand2 == (int)v.index_) {
          snap_rec = &snap;
          break;
        }
      }
      if (snap_rec &&
        ...
        const auto& orig_stroke1 = stroke_graph.orig_strokes_[snap_rec->orig_a.first];
        const auto& orig_stroke2 = stroke_graph.orig_strokes_[snap_rec->orig_b.first];
        ...
        // Turn angles.
        const auto angle1 = std::acos(std::clamp(tangent1.dot(connection), 0.0, 1.0));
        const auto angle2 = std::acos(std::clamp(tangent2.dot(-connection), 0.0, 1.0));
        if (angle2 < angle1) {
          assert((stroke_graph.vertices_[vi].p_ - snap_rec->p_b).squaredNorm() <=
          ...
          // If edge1's size is <= 2, then some other connection got made in the middle of
          // the connection. In this case, we can't fix up the vertex location without
          // majorly reworking the topology.
          if (edge1.size() > 2) {
...
void build_plane_graph(span<const Stroke> strokes, StrokeGraph& stroke_graph) {
  ...
  stroke_graph.orig_bvh_ = std::make_unique<PolylineBVH>(stroke_graph.orig_strokes_);
...
StrokeSnapInfo increment_strokes(StrokeGraph& graph, span<const Stroke> strokes,
  ...
  for (size_t i = start_idx; i < end_idx; ++i) {
    snap_info = add_stroke_incremental_topological(graph, &cache, strokes[i], false);
  }
  ...
  if (to_bridge) {
    const auto bridge_info = find_bridge_locations(graph);
    augment_with_bridges(graph, bridge_info);
  }
...
static void vanilla_candidates(const StrokeGraph& end_stroke_graph,
                               ...
                               bool generate_corners = false, int num_cand = -1) {
  ...
  if (generate_corners) {
    ...
    for (size_t vi = 0; vi < stroke_graph.vertices_.size(); ++vi) {
      if (!stroke_graph.vertex(vi).is_active() || stroke_graph.vertex(vi).valence() != 1)
        continue;
      ...
      bool is_end_dangling = false;
      {
        ...
        if (convert_strokes2orig(stroke_graph, st)) {
          if (convert_orig2strokes(end_stroke_graph, st) &&
              (std::abs(st.second) < 1e-10 || std::abs(st.second - 1.0) < 1e-10)) {
            auto end_v =
              endpoint_to_vertex(end_stroke_graph, Endpoint(st.first, st.second < 0.5));
    ...
    for (size_t i = 0; i < stroke_pos_candidates.size(); ++i) {
      ...
      if (!trial_add_corner_junction(end_stroke_graph, stroke_graph, junc_round, junc,
                                     s_junc, tmp_corner_predictions) &&
          edge_length > 1.5 * orig_length) {
        junc_round = s_junc;
        junc_round.points[0].second = (junc_round.points[0].second < 0.5) ? 1.0 : 0.0;
        ok = stroke_to_original_stroke_indexing(stroke_graph, junc_round);
        trial_add_corner_junction(end_stroke_graph, stroke_graph, junc_round, junc,
                                  s_junc, tmp_corner_predictions);
      }
    }
  }
  ...
  // 2. Generate other candidates
  {
    ...
    for (size_t vi = 0; vi < stroke_graph.vertices_.size(); ++vi) {
      ...
      snap_candidates(stroke_graph, vi, &cache, feature_type, tmp_stroke_pos_candidates,
                      van_num_cand, false, train_time);
    ...
    for (const auto& snap : stroke_pos_candidates) {
      ...
      const auto v1 = endpoint_to_vertex(
        stroke_graph, Endpoint(junc.points[0].first, junc.points[0].second == 0));
      const auto v2 = endpoint_to_vertex(
        stroke_graph, Endpoint(junc.points[1].first, junc.points[1].second == 0));
      ...
      size_t orig_sid = (end_d1 == 1) ? junc.points[1].first : junc.points[0].first;
      ...
      bool unseen_junc =
        ...
        std::find_if(tmp_corner_predictions.begin(), tmp_corner_predictions.end(),
                     [&junc](const Junction& j) {
                       return (j.type == junc.type) &&
                              (j.points[0].first == junc.points[0].first) &&
                              (j.points[0].second == junc.points[0].second) &&
      ...
      if (unseen_junc && (end_d0 >= 0 && end_d0 <= 1) && (end_d1 >= 0 && end_d1 <= 1) &&
          (end_d0 == 1 || end_d1 == 1) && graph_visible(end_stroke_graph, junc)) {
        junc.orig_dist = junction_distance_init(stroke_graph, junc);
        dangling_predictions.emplace_back(junc);
      } else if (generate_corners && unseen_junc && (end_d0 > 1 || end_d1 > 1) &&
                 (end_d0 == 1 || end_d1 == 1) && graph_visible(end_stroke_graph, junc)) {
        if (end_d1 > 1)
          std::swap(junc.points[0], junc.points[1]);
  ...
  for (auto& junc : tmp_corner_predictions) {
    ...
    const auto junc_p = std::make_pair(std::min(vid1, vid2), std::max(vid1, vid2));
...
void vanilla_candidates(const StrokeGraph& stroke_graph,
  ...
  vanilla_candidates(stroke_graph, stroke_graph, FeatureType::OrigStroke,
                     dangling_predictions, corner_predictions, train_time, false,
                     num_cand);
}
...
void vanilla_candidates(const StrokeGraph& end_stroke_graph,
  ...
  vanilla_candidates(end_stroke_graph, stroke_graph, FeatureType::OrigStroke,
                     dangling_predictions, corner_predictions, train_time, false, -1);
}

void predicted_corner_candidates(const StrokeGraph& end_stroke_graph,
                                 const StrokeGraph& stroke_graph,
                                 const FeatureType feature_type,
                                 std::vector<Junction>& corner_predictions, bool to_dedup,
                                 bool include_prev_connections) {
  prediction_feature_type = feature_type;

  // 1. Generate the prototype corner candidates
  std::vector<Junction> dangling_predictions, proto_corner_predictions;
  vanilla_candidates(end_stroke_graph, stroke_graph, FeatureType::OrigStroke,
                     dangling_predictions, proto_corner_predictions, false, true, -1);

  // Consider the connections in the intermediate graph for the corner generation
  if (include_prev_connections) {
    ...
    dangling_predictions.clear();
    vanilla_candidates(end_stroke_graph, end_stroke_graph, FeatureType::OrigStroke,
                       dangling_predictions, conn_proto_corner_predictions, false, true,
                       -1);
    ...
    for (auto& junc : conn_proto_corner_predictions) {
      ...
      if (std::find(proto_corner_predictions.begin(), proto_corner_predictions.end(),
        ...
        bool connection_check = connection_corner_visible(end_stroke_graph, junc);
        if (connection_check)
  ...
  // 2. Expand
  expand_corner_candidates((include_prev_connections) ? end_stroke_graph : stroke_graph,
                           proto_corner_predictions, corner_predictions);

  // 3. Predict
  update_high_valence_junction_predictions(
    (include_prev_connections) ? end_stroke_graph : stroke_graph, corner_predictions,
    prediction_feature_type, to_dedup);
}
...
void complete_predicted_corner_candidates(const StrokeGraph& end_stroke_graph,
  ...
  predicted_corner_candidates(end_stroke_graph, stroke_graph, feature_type,
                              corner_predictions, false, include_prev_connections);
  ...
  // 5. Complete
  {
    ...
    to_complete_candidates.insert(to_complete_candidates.end(),
                                  corner_predictions.begin(), corner_predictions.end());
    complete_graph_candidates(end_stroke_graph, stroke_graph.orig_strokes_, stroke_graph,
                              stroke_graph.orig_strokes_.size() - 1,
...
void expand_corner_candidates(const StrokeGraph& stroke_graph,
  ...
  for (const auto& junc : corner_predictions) {
    ...
    assert((((expanded_juncs.empty()) ? junc.type : expanded_juncs.front().type) ==
              JunctionType::R &&
    ...
    expanded_corner_predictions.insert(expanded_corner_predictions.end(),
                                       expanded_juncs.begin(), expanded_juncs.end());
  }
}

void connect_graph(StrokeGraph& stroke_graph, std::vector<Junction>& candidates) {
  // Connect. Note that we are hard-coded to use straight line connection here. All
  // connections shouldn't introduce new intersections since they already passed the
  // visibility check.
...
void viz_pdf(span<const Stroke> strokes, const StrokeGraph& graph,
             const std::string& path,
             const std::vector<ClassifierPrediction>& predictions,
             const std::vector<std::string>& connected_junc_strs,
             span<const int> face_colors, bool plot_skeleton) {
  std::vector<std::pair<ClassifierPrediction, VizProbabilities>> viz_predictions;
  viz_predictions.reserve(predictions.size());
  for (auto const& pred : predictions) {
    viz_predictions.emplace_back(pred, VizProbabilities());
  }

  viz_pdf(strokes, graph, path, viz_predictions, connected_junc_strs, face_colors,
          plot_skeleton);
}
...
void viz_pdf(
  ...
  const std::vector<std::string>& connected_junc_strs, span<const int> face_colors,
  bool plot_skeleton) {
  PlotParams params;
  params.media_box = visual_bounds(strokes);
  params.compress = false; // to be fast
  std::vector<ClassifierPrediction> pred;
  pred.reserve(predictions.size());
  for (const auto& p : predictions) {
    pred.emplace_back(p.first);
  }
  params.viz_centerlines = plot_skeleton;
  params.viz_envelopes = plot_skeleton;
  save_pdf(graph, path, params, pred, face_colors);

  // Write probabilities to a txt file
  size_t pos = path.find(".pdf");
  std::string txt_path = path;
  txt_path.replace(pos, path.length(), ".txt");
  std::ofstream ofs;
  ofs.open(txt_path);
  for (size_t i = 0; i < predictions.size(); ++i) {
    std::stringstream ss;
    ss << std::setprecision(2) << "(" << predictions[i].first.orig_a.first << ", "
       << predictions[i].first.orig_a.second << " - " << predictions[i].first.orig_b.first
       << ", " << predictions[i].first.orig_b.second << ")";

    if (predictions[i].second.prob_ >= 0)
      ofs << i << ": " << ss.str() << "\t"
          << ((predictions[i].first.key.type == JunctionType::R) ? "R" : "T") << "\t"
          << predictions[i].second.prob_ << "\t" << predictions[i].second.future_prob_
          << "\t" << predictions[i].second.env_dist_ << std::endl;
    else
      ofs << i << ": " << ss.str() << "\t"
          << ((predictions[i].first.key.type == JunctionType::R) ? "R" : "T") << "\t"
          << predictions[i].first.prob << "\t" << predictions[i].second.future_prob_
          << "\t" << predictions[i].second.env_dist_ << std::endl;
  }
  for (size_t i = 0; i < connected_junc_strs.size(); ++i) {
    ofs << connected_junc_strs[i] << std::endl;
  }
  ofs.close();
}
...
void viz_connections(const GraphState& state,
  ...
  for (size_t i = 0; i < state.candidates_.size(); ++i) {
    ...
    junc_ss << std::setprecision(2) << "(" << state.candidates_[i].points[0].first << ", "
  ...
  for (size_t i = 0; i < graph.strokes_.size(); ++i)
    for (const auto& [t, vid] : graph.strokes2vid_[i]) {
      ...
      if (graph.strokes_[i].size() > 0) {
        Vec2 vec(graph.strokes_[i].pos_norm(t).x(), graph.strokes_[i].pos_norm(t).y());
