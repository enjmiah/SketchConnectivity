...
Bitmap rasterize(const span<const Stroke> strokes, const int width, const int height,
                 BoundingBox bb) {
  const auto scale = rasterize_scale(strokes, width, height, bb);

  auto* surface = cairo_image_surface_create(CAIRO_FORMAT_A8, width, height);
  auto* cr = cairo_create(surface);
  cairo_set_antialias(cr, CAIRO_ANTIALIAS_NONE);
  cairo_set_line_width(cr, 0);
  auto vertices = std::vector<CoordMat>();
  for (const auto& stroke : strokes) {
    outline_to_polygons(stroke, vertices);
  }
  for (const auto& mat : vertices) {
    const auto n = mat.rows();
    auto moved_to = false;
    for (Index i = 0; i < n; ++i) {
      const Vec2 xy = (from_eigen(mat.row(i)) - Vec2(bb.xMid(), bb.yMid())) * scale +
                      Vec2(width / 2, height / 2);
      if (moved_to) {
        cairo_line_to(cr, xy.x(), xy.y());
      } else {
...
ColorBitmap rasterize_regions(const StrokeGraph& graph, const int width, const int height,
                              const span<const int> face_colors) {
  const auto bb = visual_bounds(graph.orig_strokes_);
  ...
  for (size_t fi = 1; fi < graph.faces_.size(); ++fi) {
    for (const auto& hi : graph.faces_[fi].cycles_) {
      ...
      for (size_t i = 0; i < n; ++i) {
        // The next two lines yield warning C4723: potential divide by 0.
        const auto x = (vertices[i].x_ - bb.xMin()) * (width / bb.width());
        const auto y = (vertices[i].y_ - bb.yMin()) * (height / bb.height());
        if (i == 0) {
    ...
    assert(face_colors[fi] < (int)ARRAY_SIZE(palette));
  ...
  const auto* data = cairo_image_surface_get_data(surface);
  const auto stride = cairo_image_surface_get_stride(surface);
  auto out = ColorBitmap(height, width);
  for (auto y = 0; y < height; ++y) {
    const auto* row = (uint32_t*)(data + y * stride);
    for (auto x = 0; x < width; ++x) {
      out(y, x) = row[x];
    }
  }
...
ColorBitmap rasterize_region_indices(const StrokeGraph& graph, const int width,
                                     const int height,
                                     const span<const int> face_colors) {
  const auto bb = visual_bounds(graph.orig_strokes_);
  auto* surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
  ...
  for (size_t fi = 1; fi < graph.faces_.size(); ++fi) {
    ...
    auto color = Col3();
    color.b = (fi & 0xff) / 255.0;
    color.g = ((fi & 0xff00) >> 8) / 255.0;
    color.r = ((fi & 0xff0000) >> 16) / 255.0;
    cairo_set_source_rgb(cr, color.r, color.g, color.b);
    cairo_fill(cr);
  }
  cairo_destroy(cr);
...
Bitmap rasterize_vertices(span<const Stroke> strokes, const int width, const int height,
                          const Float radius_increase_px) {
  auto bb = BoundingBox();
  const auto scale = rasterize_scale(strokes, width, height, bb);
  ...
  for (const auto& stroke : strokes) {
    ...
    // Sometimes the width gets really small at the end but it's not representative.
    const auto w0 = std::max(0.5 * stroke.width(0), 0.33 * pen_width);
    // const auto w0 = 0.5 * pen_width;
    cairo_arc(cr, scale * (stroke.x(0) - bb.xMid()) + width / 2,
              scale * (stroke.y(0) - bb.yMid()) + height / 2,
              scale * w0 + radius_increase_px, 0, 2 * M_PI);
    cairo_close_path(cr);
    cairo_fill(cr);
    const auto e = stroke.size() - 1;
    const auto we = std::max(0.5 * stroke.width(e), 0.33 * pen_width);
    // const auto we = 0.5 * pen_width;
    cairo_arc(cr, scale * (stroke.x(e) - bb.xMid()) + width / 2,
              scale * (stroke.y(e) - bb.yMid()) + height / 2,
              scale * we + radius_increase_px, 0, 2 * M_PI);
    cairo_close_path(cr);
    cairo_fill(cr);
  }
  cairo_destroy(cr);
