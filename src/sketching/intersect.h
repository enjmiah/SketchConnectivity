...
/**
 * Positive iff triangle pqr winds counter-clockwise in a Y-up system.  Falls back to
 * robust arithmetic if necessary.
 */
...
[[nodiscard]] inline bool segment_intersection( //
  const Vec2& a, const Vec2& b, const Vec2& c, const Vec2& d, double& s, double& t) {
...
/**
 * Excludes p and q from the intersections.  Useful for line of sight calculations.
 *
 * @param p Start of line segment.
 * @param q End of line segment.
 * @param stroke Stroke to intersect with.
 * @param out_s Value in [0, 1] indicating where on segment pq intersection was found.
 * @param out_t Arc length value indicating where on stroke intersection was found.
 * @return true if and only if an intersection was found.
 */
[[nodiscard]] bool intersect_segment_stroke_exclusive(Vec2 p, Vec2 q,
...
/**
 * An uneven capsule with different radii at each end.  Tapered strokes are made of these.
 */
struct Bicapsule {
  Vec2 p_a;
  Vec2 p_b;
...
/**
 * Return (signed distance, projection amount) to a bicapsule.  Projection amount is not
 * clamped to [0, 1].
 */
...
/**
 * Returns (distance from p, arc length on stroke).  Distance will be infinity if no good
 * snap point is found.  Distance is the centerline distance.
 *
 * @param to_snap Stroke containing endpoint to snap.
 * @param head True iff you are trying to snap the head, otherwise it is assumed that you
 *             are trying to snap the tail.
 * @param other BVH node containing the stroke to consider snapping to.
 * @param out_closest_env_dist Will be set to the smallest envelope distance from the
 *                             endpoint to the stroke.
 */
...
/**
 * Cast a ray in direction `r` from point `p`, and return all points which intersect the
 * ray.  The results are sorted by distance from `r`.
 */
