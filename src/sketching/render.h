...
Bitmap rasterize(span<const Stroke> strokes, int width, int height, BoundingBox bb);

Bitmap rasterize_vertices(span<const Stroke> strokes, int width, int height,
                          Float radius_increase_px);
...
/** Each pixel is stored as packed ARGB components in a 32-bit uint. */
ColorBitmap rasterize_regions(const StrokeGraph&, int width, int height,
                              span<const int> face_colors);
ColorBitmap rasterize_region_indices(const StrokeGraph& graph, const int width,
                                     const int height, const span<const int> face_colors);
