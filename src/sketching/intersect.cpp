...
double project_pt_on_segment(const Vec2& p, const Vec2& a, const Vec2& b) {
  auto t = ((p.x() - a.x()) * (b.x() - a.x()) + (p.y() - a.y()) * (b.y() - a.y())) /
           (square(b.x() - a.x()) + square(b.y() - a.y()));
  return std::clamp(t, 0.0, 1.0);
}

/// Return arclength t on the stroke corresponding to the closest point if there is an
/// intersection, or -infinity if there is none.
...
void intersect_segment_with_polyline(const Vec2& a, const Vec2& b,
  ...
  const auto ab_bb = BoundingBox(std::min(a.x(), b.x()), std::max(a.x(), b.x()),
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    if (hit) {
      ...
      if (u < 1.0 && v < (c - d).norm()) { // Do not include end of interval.
        const auto contender = Vec2(
          u, std::clamp(geom.arclength(i) + v, geom.arclength(i), geom.arclength(i + 1)));
...
static bool intersect_segment_stroke_exclusive(const Vec2 p, const Vec2 q,
  ...
  // Note that the BoundingBox constructor does the appropriate comparisons to make this a
  // valid bounding box.
    ...
    for (auto i = 0; i < n - 1; ++i) {
      ...
      if (hit) {
        ...
        if (sw > min_s && sw < max_s) {
          ...
          out_s = sw;
          out_t =
            clamped_lerp(geom.arclength(i), geom.arclength(i + 1), t / (p - q).norm());
          return true;
        }
      }
    }
...
void intersect_segment_stroke_exclusive(Vec2 p, Vec2 q, const PolylineBVHLeaf& stroke,
                                        std::vector<Vec2>& out_arclens) {
  const auto eps = Float(1e-5);
...
Vec2 signed_dist_bicapsule(Vec2 p, Bicapsule bicap) {
  ...
  auto q = (1 / h) * Vec2(p.dot(Vec2(bicap.p_b.y_, -bicap.p_b.x_)), p.dot(bicap.p_b));
...
Float signed_dist_stroke(const Vec2 p, const Stroke& stroke) {
  ...
  for (Index i = 0; i < n1; ++i) {
    const auto [env_dist, _u] = signed_dist_bicapsule(p, stroke_segment(stroke, i));
    sd = std::min(sd, env_dist);
  }
  return sd;
}

bool line_of_sight(const Vec2 p, const Vec2 q, span<const Stroke> strokes,
                   span<const BoundingBox> centerline_bbs) {
  assert(strokes.size() == centerline_bbs.size());
  Float s, t;
  const auto n = strokes.size();
  for (size_t i = 0; i < n; ++i) {
    if (intersect_segment_stroke_exclusive(p, q, {strokes[i], centerline_bbs[i]}, s, t)) {
      return false;
    }
  }
  return true;
...
bool line_of_sight(const Vec2 p, const Vec2 q, const PolylineBVH& bvh, const Float min_s,
                   const Float max_s) {
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    for (int j = i + 2; j < n - 1; ++j) {
      ...
      if (segment_intersection(a, b, c, d, s, t)) {
        out.emplace_back(
          std::clamp(geom.arclength(i) + s, geom.arclength(i), geom.arclength(i + 1)),
          std::clamp(geom.arclength(j) + t, geom.arclength(j), geom.arclength(j + 1)));
      }
    }
  }
...
void intersect_self(const PolylineBVHLeaf& node, std::vector<Vec2>& out_arclens) {
  return intersect_self(*node.geometry, out_arclens);
}

void intersect_different(const PolylineBVHLeaf& node1, const PolylineBVHLeaf& node2,
                         std::vector<Vec2>& out) {
  ...
  for (int i = 0; i < n1 - 1; ++i) {
    ...
    for (int j = 0; j < n2 - 1; ++j) {
      ...
      if (segment_intersection(a, b, c, d, s, t)) {
        out.emplace_back(
          std::clamp(geom1.arclength(i) + s, geom1.arclength(i), geom1.arclength(i + 1)),
          std::clamp(geom2.arclength(j) + t, geom2.arclength(j), geom2.arclength(j + 1)));
      }
    }
  }
  remove_duplicate_intersection_junctions(out);
...
void intersect_self(const EnvelopeBVHLeaf& node, std::vector<IntersectionEvent>& out) {
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    for (int j = i + 2; j < n - 1; ++j) {
      ...
      if (segment_intersection(a, b, c, d, u, v)) {
        const auto s =
          std::clamp(geom.arclength(i) + u, geom.arclength(i), geom.arclength(i + 1));
        const auto t =
          std::clamp(geom.arclength(j) + v, geom.arclength(j), geom.arclength(j + 1));
  ...
  const auto try_to_add_current_event = [&]() {
    for (auto& event : out) {
      ...
      // Note that s and t lie on the same stroke in this case, so the event's s interval
      // might correspond to the current_event's t interval, or vice versa.  Be careful
      // here...
...
void intersect_different(const EnvelopeBVHLeaf& node1, const EnvelopeBVHLeaf& node2,
  ...
  // Intersect circles around vertices of geom2 with geom1 and record contiguous runs.
  const auto& geom2 = *node2.geometry;
  const auto n2 = geom2.size();
  ...
  for (int i = 0; i < n1 - 1; ++i) {
    ...
    {
      intersect_segment_with_polyline(geom1.xy(i), geom1.xy(i + 1), node2, hits);
      for (const auto& [u, t] : hits) {
        const auto s = clamped_lerp(geom1.arclength(i), geom1.arclength(i + 1), u);
        if (current_event.centerline_intersection() &&
            // The same intersection can be found twice across different iterations due to
      ...
      hits.clear();
      // Note that a miss does not necessarily mean that we have left the intersection
      // event, since we are intersecting with the *centerline*, not the envelope.
    }

    if (i == n1 - 2) {
      // Check if the end of the segment intersects with the other stroke.
      // This is a special case for the last segment since for the other segments we do
      // not want to check vertex locations twice (one as the end of a segment and once as
      // the start).
  ...
  // We need to make the t_start and t_end values accurate, and add possibly missed
  // envelope-envelope intersections.
  const auto pw = std::max(geom1.pen_width(), geom2.pen_width());
  for (const auto& rev_event : reverse_events) {
    for (auto& event : out) {
      // Check for overlap.
      if (event.s_start <= rev_event.s_end && rev_event.s_start <= event.s_end) {
        ...
        auto broken = false;
        for (int j = 0; j < n2; ++j) {
          if (within_interval(geom2.arclength(j), attempt.t_start, attempt.t_end) &&
              !is_hit2[j]) {
            broken = true;
            break;
          }
        ...
        if (!broken) {
          for (int j = 0; j < n1; ++j) {
            if (within_interval(geom1.arclength(j), attempt.s_start, attempt.s_end) &&
                is_hit1_t[j] == -infinity) {
              broken = true;
              break;
            }
  ...
  *out_closest_env_dist = INFINITY;
  ...
  if (BoundingBox(p.x_, p.y_, radius).touches(other.bb)) {
    ...
    for (Index i = start; i < stop; ++i) {
      auto [env_dist, u] = signed_dist_bicapsule(p, stroke_segment(other_stroke, i));
      if (std::isnan(env_dist)) {
        // The bicapsule is degenerate (i.e. a circle).
        // Skipping is fine if this bicapsule occurs in the interior, because it will be
        // covered by the adjacent bicapsules, however this is a problem if this is an
        // endpoint with the end vertex having the larger radius.
        // I imagine that case is rare, so this probably okay...
        continue;
      }
      u = std::clamp(u, 0.0, other_stroke.arclength(i + 1) - other_stroke.arclength(i));
      const auto arclen =
      ...
      if (&to_snap.stroke() == &other_stroke) {
        ...
        if (!(std::max(3 * cen_dist, M_PI * to_snap.stroke().pen_width()) < arc_dist)) {
          continue; // Spurious.
        }
      }
      env_dist -= radius;
...
Vec2 find_snap_point_legacy(const Stroke& to_snap, const bool head,
  ...
  auto radius = (1 + fudge) * 0.5 * (head ? to_snap.width(0) : to_snap.width(Back));
  ...
  if (BoundingBox(p.x_, p.y_, radius).touches(other.bb)) {
    ...
    for (Index i = start; i < stop; ++i) {
      ...
      if (env_dist <= fudge * other_w) {
        // Endpoint falls within the other stroke's envelope (within fudge factor).
        goto valid;
      }
      if (env_dist > radius) {
...
std::vector<std::pair<size_t, Float>> ray_intersections(const PolylineBVH& polylines,
                                                        const Vec2& p, const Vec2& r) {
  ...
  for (size_t i = 0; i < polylines.nodes.size(); ++i) {
    ...
    for (auto j = 0; j < geom.size() - 1; ++j) {
      ...
      if (segment_ray_intersection(segstart, segend, p, r, s, t)) {
        const auto narclen =
          ((1.0 - s) * geom.arclength(j) + s * geom.arclength(j + 1)) / geom.length();
