...
Float stepaway_projection_dist(const Stroke& s1, const bool s1_head,
  ...
  const auto stepaway_location = (s1_head ? std::min(stepaway_amount, s1.length())
  ...
  auto start = std::max(connection_arclen2 - near_factor * stepaway_amount, 0.0);
  auto stop = std::min(connection_arclen2 + near_factor * stepaway_amount, s2.length());
...
Float extra_s2_length(const StrokeGraph& graph, const Stroke& s2, Float arclen2,
  ...
  if (!(interior_st.second == 0.0 || interior_st.second == 1.0)) {
    SPDLOG_DEBUG("Mapped position {} on stroke {} not at vertex", interior_st.second,
                 idx2);
  }
  ...
  do {
    if (it.index_ == he_idx) {
      ...
      dist_to_v =
        (it_other_end.second > (arclen2 / s2.length())) ? arclen2 : s2.length() - arclen2;
      break;
    }
...
Vec2 stepaway_tangent(const Stroke& s1, const bool s1_head, const Float stepaway_amount) {
  ...
  const auto stepaway =
    (s1_head ? s1.pos(std::min(stepaway_amount, furthest_arclen))
...
Float AbsEnvelopeDistance::operator()( //
  ...
  const auto w1 = fast_lerp(s1.width(iv1.first), s1.width(iv1.first + 1), iv1.second);
  const auto w2 = fast_lerp(s2.width(iv2.first), s2.width(iv2.first + 1), iv2.second);
...
Float EnvelopeDistance::operator()(const Stroke& s1, const Float arclen1, const size_t,
    ...
    case Normalization::PenWidthPairwiseMean: {
      ...
      if (graph_ptr_ && !graph_ptr_->orig_bvh_->combine_he_indices.empty()) {
        ...
        for (const auto he : graph_ptr_->orig_bvh_->combine_he_indices) {
          get_forward_chain_original_indices(*graph_ptr_, he, adjacent_stroke_indices);
        }
        double max_width = 0;
        for (const auto [sid, forward] : adjacent_stroke_indices) {
          max_width = std::max(max_width, graph_ptr_->orig_strokes_[sid].pen_width());
        }
...
Float ClosestAnyOverConnection1::operator()( //
  ...
  for (const auto& node : bvh_.nodes) {
    ...
    if (node.bb.distanceTo(p) < closest_dist && node.geometry != &s1) {
      if (node.geometry == &s1) {
        closest_dist =
          std::min(closest_dist, smart_projection_dist(s1, arclen1, s1, _proj, _s));
      } else {
        closest_dist =
...
Vec2 ClosestAnyOtherOverConnection1::closest( //
  ...
  if (!graph_ptr_) {
    ...
    auto find_adjacent_strokes = [&](const Stroke& s, size_t idx, Float arclen) {
      s.ensure_arclengths();
    ...
    // Avoid the masked original strokes (the forward or not doesn't matter here)
  ...
  force_assert((!graph_ptr_ || graph_ptr_->orig_bvh_) && "Need to initialize orig_bvh_.");
  const auto& bvh = (graph_ptr_ ? *graph_ptr_->orig_bvh_ : bvh_);
  const auto blocking_bvh_node =
    (idx2 == (size_t)-1 ? PolylineBVHLeaf(s2, bounds(s2)) : bvh.nodes[idx2]);
  for (size_t i = 0; i < bvh.nodes.size(); ++i) {
    const auto& node = bvh.nodes[i];
    // TODO: Should we do something special in the node.geometry == &s1 case?
    if (node.bb.distanceTo(p) < closest_dist && //
        std::find_if(adjacent_stroke_indices.begin(), adjacent_stroke_indices.end(),
                     [=](const std::pair<size_t, bool>& p) { return p.first == i; }) ==
          adjacent_stroke_indices.end()) {
      node.geometry->ensure_arclengths();
      const auto dist = closest_point(*node.geometry, p, proj, _s);
      if (dist < closest_dist &&

          // At a graph vertex, we want to avoid having the closest distance be 0.
          (proj - p).squaredNorm() > 1e-10 && (proj - q).squaredNorm() > 1e-10) {

        if (!(limit_to_visible_ &&
              intersect_segment_stroke_exclusive(p, proj, blocking_bvh_node, _u, _v))) {
          closest_dist = dist;
          closest_p = proj;
        } else {
...
Float OtherEndpointClosestAnyEnvOverEnvConnection1::operator()( //
  ...
  const auto other_end_index = (arclen1 < 0.5 * s1.length() ? s1.size() - 1 : 0);
  ...
  for (const auto& node : bvh_.nodes) {
    ...
    if (node.bb.distanceTo(other_end) < closest_dist && node.geometry != &s1) {
      closest_dist = std::min(
        closest_dist, signed_dist_stroke(other_end, *node.geometry) - other_end_radius);
      if (closest_dist <= 0.0) {
        return 0.0;
      }
    }
...
Float StepawayOverConnection1::operator()( //
  ...
  if (graph_ptr_) {
    for (const auto he : graph_ptr_->orig_bvh_->combine_he_indices) {
      ...
      auto proj_dist =
        stepaway_projection_dist(s1, /*s1_head=*/arclen1 < 0.5 * s1.length(),
                                 /*stepaway_amount=*/centerline_dist,
                                 graph_ptr_->orig_strokes_[sid], orig_st.second);
...
void TangentAngle::init(const PolylineBVH& bvh) {
  ...
  // TODO: This is really inefficient; we only need to compute tangents for strokes that
  ...
  for (auto i = decltype(n){0}; i < n; ++i) {
    ...
    if (stroke.size() <= 1) {
      ...
      m_tangents.block(i, 0, 1, 2) =
        to_eigen(bezier[0].normalized_head_tangent()).transpose();
      bezier.clear();
      fit_bezier_spline(ConstStrokeView(stroke, stroke.size() - 10, stroke.size()), 3.0,
                        bezier);
      m_tangents.block(i, 2, 1, 2) =
        to_eigen(bezier.back().normalized_tail_tangent()).transpose();
    }
...
Float InteriorTangentAngle::operator()( //
  ...
  if ((p1 - p2).squaredNorm() < 1e-10) {
    ...
    const auto angle = std::acos(std::clamp(connection.dot(avg_tangent), -1.0, 1.0));
    assert(!std::isnan(angle));
    // Unlike endpoint tangents, the interior tangent angle doesn't have a specific
    // orientation.
...
const BezierSpline& InteriorTangentAngle::smooth_fit(const Stroke& stroke,
...
Float TangentAngle1::operator()(const Stroke& s1, Float arclen1, size_t idx1,
  ...
  if ((p1 - p2).squaredNorm() < 1e-10) {
    ...
    const auto tangent = from_eigen(
      m_tangents.block(idx1, (arclen1 < 0.5 * s1.length() ? 0 : 2), 1, 2).transpose());
...
Float TangentAngle2::operator()(const Stroke& s1, Float arclen1, size_t /*idx1*/,
                                const Stroke& s2, Float arclen2, size_t idx2) const {
  const Vec2 p1 = s1.pos(arclen1);
  const Vec2 p2 = s2.pos(arclen2);
  if ((p1 - p2).squaredNorm() < 1e-10) {
    ...
    const auto tangent = from_eigen(
      m_tangents.block(idx2, (arclen2 < 0.5 * s2.length() ? 0 : 2), 1, 2).transpose());
    const auto angle = std::acos(std::clamp(v.dot(tangent), -1.0, 1.0));
    assert(!std::isnan(angle));
    return angle;
  }
...
Float ClosestDistanceOnExtension1::operator()( //
  ...
  if ((p - q).squaredNorm() < 1e-10) {
    ...
    Float s, t;
    if (intersect_segment_stroke_exclusive(p, p + max_step * centerline_dist * tangent,
                                           bvh_.nodes[idx2], s, t)) {
      return 0.0;
    }
    ...
    for (const auto step : {0.5, 1.0, max_step}) {
      const auto new_dist = closest_point(
        bvh_.nodes[idx2], p + step * centerline_dist * tangent, closest_dist, proj, s);
...
Float Busyness1::operator()( //
  ...
  if (idx2 == (size_t)-1) {
    // We want to pretend there is no endpoint at the dissolved stroke, so we reverse
    // the effects of that.
    busyness -= gaussian_factor((p - s2.pos(arclen2)).norm() / average_width(s2),
                                busyness_falloff_);
  }
  return busyness;
}
...
Float ConnectedDistanceToEndpoint::operator()(const Stroke& /*s1*/, Float /*arclen1*/,
  ...
  {
    std::vector<std::pair<size_t, bool>> adjacent_stroke_indices;
    get_forward_chain_original_indices(*graph_ptr_, he_idx, adjacent_stroke_indices);
    std::unordered_set<size_t> seen_sid;
    for (const auto [sid, forward] : adjacent_stroke_indices) {
      if (seen_sid.count(sid))
        continue;
...
Float DrawingOrder::operator()( //
  ...
  if (idx1 == idx2) {
    return arclen1 == 0.0; // Head trying to connect to tail. Head is drawn first.
  }
