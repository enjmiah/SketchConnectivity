...
void copy_segment(const Stroke& from, Stroke& to, const double start_index,
  ...
  if (r > 0.0) {
    ...
    if (to.xy(i).isApprox(to.xy(i - 1))) {
      // Need to avoid creating duplicate vertices when lerping with tiny factors.
      to.resize(to.size() - 1);
    }
  }
}
...
void Stroke::compute_arclengths() const {
  ...
  if (n_points > 0) {
    ...
    for (Index i = 1; i < n_points; ++i) {
      const auto dist = std::sqrt(square(x_[i] - x_[i - 1]) + square(y_[i] - y_[i - 1]));
      if (!warned && dist == 0.0) {
...
void Stroke::reserve(Index new_capacity) {
  if (new_capacity > capacity_) {
    auto new_memory = std::make_unique<Float[]>(new_capacity * (has_time() ? 4 : 3));
...
Stroke Stroke::fractional_slice(Float start, Float stop) const {
  auto out = clone(); // TODO: This is inefficient because it overallocates capacity.
  out.trim(start, stop);
  return out;
}
...
void Stroke::split(const span<const Float> split_values,
  ...
  assert(!(out_splits.data() <= this && this < out_splits.data() + out_splits.size()));
  ...
  for (std::size_t split_index = 1; split_index < n_split_values; ++split_index) {
...
// In debug mode, MSVC normally doesn't optimize `do { blah; } while(0)` to `blah;`.  :(
