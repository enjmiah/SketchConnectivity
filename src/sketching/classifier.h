...
struct ClassifierPrediction {
  ...
  struct {
    ...
    /// Index of a vertex.
    int cand1 = 0;
    /// Either index of a vertex (for end-end) or a stroke index (for end-stroke).
    int cand2 = 0;
  } key;
  Vec2 p_a; ///< Position of cand1 at prediction time.
  Vec2 p_b; ///< Position of cand2 at prediction time.
  /// Location of cand1 on the original drawing as (index, normalized arc length).
  StrokeTime orig_a{0, 0};
  /// Location of cand2 on the original drawing as (index, normalized arc length).
 ...
 *
 * This function supports high-valence junctions, and works for both vertex-vertex and
 * vertex-stroke connections.
 *
 * @param graph The drawing to operate on.  cand1 and cand2 are expected to index into the
 *              graph's strokes_ array (rather than orig_strokes_).
 * @param bvh BVH for the stroke graph.
 * @param cand1 Connecting vertex, expressed as an endpoint.  High-valence endpoints will
 *              be handled for you, meaning that a single connection probability may be
 *              the aggregation of multiple classifier evaluations.
 * @param cand2 Connected-to point, as (stroke index, normalized arc length).  For
 *              connecting to a vertex, the normalized arc length value must be exactly 0
 *              or 1.  High-valence endpoints will be handled for you.
 * @param out_prob Array to output probabilities to.
 * @param out_predictions Optional extra information about each classifier evaluation.
 * @param expand Expand the candidates.
 */
void compute_junction_probabilities(
  const StrokeGraph& graph, span<const Endpoint> cand1, span<const StrokeTime> cand2,
  FeatureType feature_type, span<Float> out_prob,
  std::vector<ClassifierPrediction>* out_predictions = nullptr, bool expand = true,
...
/**
 * Return the number of classifier evaluations needed to assess the probability of a
 * potentially high-valence connection between two vertices.
 */
Index n_classifier_evaluations_vv(const StrokeGraph& graph, const Endpoint e1,
                                  const Endpoint e2);
/**
 * Return the number of classifier evaluations needed to assess the probability of a
 * potentially high-valence connection between a vertex and the interior of an edge.
 */
...
void compute_features(const StrokeGraph& graph, span<const Endpoint> cand1,
                      span<const StrokeTime> cand2, span<FeatureVector> out_features,
                      const std::vector<JunctionType::Type>& junction_types =
                        std::vector<JunctionType::Type>());
// Run on stroke
